...
button_colors = {
    "pink_button": (255, 192, 203),  # Example color for a pink button
    "blue_button": (0, 0, 255),      # Example color for a blue button
    "white_button": (255, 255, 255)  # White color for a white button
}
...
drawing = False
last_pos = None
global_cell_index = None 

# Initialize global variables
current_color = YELLOW  # Start with the indicative circle as yellow
fingertip_position = None
drawn_positions = []  # List to hold drawn positions
color_change_time = None  # Time of the last color change


def main():
    global global_cell_index, fingertip_position
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(
                ...
                if hand_size < SCREEN_INTERACTION_THRESHOLD and screen_grid is not None:
                    for cell in screen_grid:
                        ...
                        if cell_x1 <= fingertip_x <= cell_x2 and cell_y1 <= fingertip_y <= cell_y2:
                            # Map to Pygame canvas coordinates
                            scale_x = 1900 / (x2 - x1)
                            scale_y = 1000 / (y2 - y1)
                            pygame_fingertip_x = (fingertip_x - x1) * scale_x
                            pygame_fingertip_y = (fingertip_y - y1) * scale_y

                            # Flip the x-coordinate
                            pygame_fingertip_x = 1900 - pygame_fingertip_x

                            fingertip_position = (int(pygame_fingertip_x), int(pygame_fingertip_y))
                            break

                if BUTTON_INTERACTION_THRESHOLD[0] <= hand_size <= BUTTON_INTERACTION_THRESHOLD[1]:
                    for det in current_detections:
                        x1, y1, x2, y2, class_name, _ = det
                        if "boton" in class_name:
...
def draw_interface(screen, color_squares, save_button_rect, drawn_positions, fingertip_position, current_color):
    screen.fill(GREY)  # Fill the background
    ...
    pygame.draw.rect(screen, GREY, save_button_rect)
    font = pygame.font.Font(None, 36)
    text = font.render('Save', True, WHITE)
    text_rect = text.get_rect(center=save_button_rect.center)
    screen.blit(text, text_rect)

    # Draw all previous positions and lines
    for item in drawn_positions:
        if len(item) == 2:  # Single position
            pos, color = item
            pygame.draw.circle(screen, color, pos, 5)
        elif len(item) == 3:  # Continuous line
            start_pos, end_pos, color = item
            pygame.draw.line(screen, color, start_pos, end_pos, 5)

    # Draw the indicative circle (mouse cursor)
    if fingertip_position:
        pygame.draw.circle(screen, current_color, fingertip_position, 20)

def pygame_thread():
    global drawn_positions, fingertip_position, current_color
    pygame.init()
    window_width, window_height = 1900, 1000
    screen = pygame.display.set_mode((window_width, window_height))
    pygame.display.set_caption("Signature Drawing App")

    # Define the color squares and save button
    color_squares = {
    ...
    save_button_rect = pygame.Rect(window_width - 220, 10, 200, 50)

    last_pos = None  # Store the last position for continuous drawing
    running = True
    while running:
        ...
        # Check if the indicative circle is over a color square to change the drawing color
        color_changed = False
        if fingertip_position:
            for color, rect in color_squares.items():
                if rect.collidepoint(fingertip_position):
                    current_color = color
                    color_changed = True
                    break

        # Continuous drawing logic
        if fingertip_position and not color_changed:
            if last_pos is not None:
                drawn_positions.append((last_pos, fingertip_position, current_color))
            last_pos = fingertip_position
        elif color_changed:
            last_pos = None

        # Draw the interface
        draw_interface(screen, color_squares, save_button_rect, drawn_positions, fingertip_position, current_color)
...
def create_grid(x1, y1, x2, y2, rows=32, cols=32):
    grid = []
